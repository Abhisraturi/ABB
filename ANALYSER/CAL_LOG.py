...
INPUT_STATUS_REGS = {
    1201: {"name": "ZERO SPAN", "delay": 120, "capture": ["N2O", "NO", "O2"]},
    ...
    def run(self):
        while not self.stop_event.is_set():
            try:
                ...
                self.conn.commit()
                print(f"✅ Inserted {len(rows)} rows into SQL table {self.table}")
...
def evaluate_value(name: str, val: float, range_client: SafeModbusClient, cal_type: str | None = None):
    ...
    if cal_type == "ZERO SPAN":
        expected = 0.0
        min_range = -0.2
        max_range = 0.2
        print(f"{name} ZERO SPAN active. Using fixed range: [0.0, 0.2]")
    else:
    ...
    for param in params:
        ...
        if val is not None:
            ...
            try:
                ...
                print(f"{param} | Value: {val:.3f} | Expected: {exp_s} | Min: {min_s} | Max: {max_s} | Error: {error:.3f} | Accuracy: {accuracy:.3f} | Status: {status}")
            except Exception:
                print(f"{param} | Value: {val} | Expected: {expected} | Min: {min_range} | Max: {max_range} | Error: {error} | Accuracy: {accuracy} | Status: {status}")
            ...
            print(f"{param} | Failed to read sensor value")
    return results
...
class CaptureWorker(threading.Thread):
    ...
    def run(self):
        print(f"▶️ {self.cal_name} triggered. Waiting {self.delay_s} seconds before capture...")
        ...
        while not self.stop_event.is_set():
            ...
            if not status:
                print(f"⏹ {self.cal_name} trigger ended.")
                break
...
class TriggerMonitor(threading.Thread):
    ...
    def run(self):
        print("🟢 Monitoring calibration input triggers...")
        while not self.stop_event.is_set():
